...
const TrainList = () => {
  ...
  const [searchClicked, setSearchClicked] = useState(false); // New state for search trigger
  ...
  useEffect(() => {
    if (searchClicked) {
      if (fromStation && toStation && selectedDate) {
        const dayOfWeek = new Date(selectedDate).getDay(); // Get day of the week from selected date
        ...
        const filtered = trainDetails.filter((train) => {
          ...
          // Check if train runs on the selected day of the week
          const runsOnDay = train.runs_on[dayOfWeek] === 1;

          return isOnRoute && runsOnDay;
        });
      ...
      setSearchClicked(false); // Reset the search trigger
    }
  }, [searchClicked, fromStation, toStation, selectedDate]);
    ...
    <div>
      ...
      <div className="mainDiv">
        ...
        <div className="listOfTrain">
          ...
          {searchClicked && filteredTrains.length === 0 && (
